...
// Date and time
const DATE_TIME_REGEX = /on (\d{1,2}\/\d{1,2}\/\d{2,4})\s*at (\d{1,2}:\d{1,2}\s*[AP]M)/;

// Merchant names
const MERCHANT_REGEX_1 = /(?:from|sent to|paid to|bought Ksh[\d,]+\.\d{1,2} of) (.*?) on \d{1,2}\/\d{1,2}\/\d{1,2} at/;
const MERCHANT_REGEX_2 = /confirmed\. (.*?) of transaction/;
const MERCHANT_REGEX_3 = /[AP]MWithdraw Ksh[\d,]+\.\d{1,2} from (.*?) New/;

// Amount
const AMOUNT_REGEX_1 = /[cC]onfirmed\.\s*(?:You have received\s|You bought\s)?Ksh([\d,]+\.\d{1,2})/;
...
const IS_BUY_GOODS_REGEX = /Ksh[\d,]+\.\d{1,2} paid to .* on \d{1,2}\/\d{1,2}\/\d{2,4}/;
...
const IS_INCOMING_REGEX = /sent to|paid to|debited from|[AP]MWithdraw|You bought/;
...
const extractTransactionDetails = (message: string): TransactionDetails => {
  ...
  const transactionCost = txCostMatch ? parseFloat(txCostMatch[1].replace(/,/g, "")) : 0.0;
